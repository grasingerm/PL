...
int main(int argc, char *argv[]) {
  ...
  for (unsigned k = 0; k < num_steps; ++k) {
    ...
    if (myid != 0) {
      MPI_Send(&uk[1], 1, MPI_DOUBLE, lnbr, 1, MPI_COMM_WORLD);
    }

    if (myid != num_procs-1) {
      MPI_Send(&uk[num_points], 1, MPI_DOUBLE, rnbr, 2, MPI_COMM_WORLD);
    }

    if (myid != 0) {
      MPI_Recv(&uk[0], 1, MPI_DOUBLE, lnbr, 2, MPI_COMM_WORLD, &status);
    }

    if (myid != num_procs-1) {
      MPI_Recv(&uk[num_points+1], 1, MPI_DOUBLE, rnbr, 1, MPI_COMM_WORLD, 
               &status);
    }
