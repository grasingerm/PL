...
potential(L::Real, x::Real) = (abs(x) < L / 2.0) ? 0.0 : Inf;
...
Z1 = exp(-beta * potential(L, x0)) / exp_x0;
println("<x>    =   $(exp_x)");
println("<x^2>  =   $(exp_xsq)");
...
if pa["plot-histogram"]
  ...
  PyPlot.plot(xline, 
              map(x -> exp(-beta * potential(L, x)) / Z_an, xline),
             "k--"; linewidth=8);
  legend(["MCMC", "Analytical"]);
  xlabel("\$x\$");
  ylabel("\$P(x)\$");
  title("Square well, \$L = $L\$; probability density of \$x\$");
  show();
end
