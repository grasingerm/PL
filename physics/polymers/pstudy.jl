...
for k1 in k1s, k2 in k2s, E0 in E0s, beta in betas, 
  ...
  println("$k1 $k2 $E0 $beta $phi0 $delta, $counter");
  ...
  #try
    ...
    if rand([true, false])
      run(pipeline(`julia amorphous_dielectric_3d.jl --k1 $k1 --k2 $k2 --E0 $E0 --beta $beta --phi $phi0 --delta $delta --num-steps $num_steps`, stdout=fname));
    else
      run(pipeline(`julia amorphous_dielectric_3d.jl --k1 $k1 --k2 $k2 --E0 $E0 --beta $beta --phi $phi0 --delta $delta --num-steps $num_steps -A`, stdout=fname));
    end
