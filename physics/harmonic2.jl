...
@add_arg_table s begin
  "--x0", "-X"
    ...
    default = 0.0
  "--spring-const-1", "-a"
  help = "a in V(x) = ax^2 - bx"
    arg_type = Float64
    default = 1.0
  "--spring-const-2", "-b"
    help = "b in V(x) = ax^2 - bx"
    arg_type = Float64
    default = 2.0
  "--beta", "-B"
    help = "1 / kT"
...
potential(a::Real, b::Real, x::Real) = a * x*x - b * x;

const x0 = pa["x0"];
const a = pa["spring-const-1"];
const b = pa["spring-const-2"];
const beta = pa["beta"];
...
px0 = counter / nsteps;
Z_an = (sqrt(pi / (a * beta))) * exp(b^2 * beta / (4 * a));
exp_x0 = counter / (nsteps * 2 * dx);
Z1 = exp(-beta * potential(a, b, x0)) / exp_x0;
println("<x>    =   $(exp_x)");
println("<x^2>  =   $(exp_xsq)");
...
if pa["plot-histogram"]
  ...
  PyPlot.plot(xline, 
              map(x -> exp(-beta * potential(a, b, x)) / Z_an, xline),
             "k--"; linewidth=8);
  legend(["MCMC", "Analytical"]);
  xlabel("\$x\$");
  ylabel("\$P(x)\$");
  title("\$V(x) = $a x^2 - $b x\$, probability density of \$x\$");
  show();
end
