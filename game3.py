...
def init(data):
    ...
    data.playerY = (data.height - data.groundY) + data.lineHeight  # player's bottom edge (distance above the base line)
    data.playerU = 0
    data.playerUMin = -7
    data.playerUMax = 7
    data.playerV = 0
    ...
    data.walls = 3
    data.wallX = [220, 240, 280]
    data.wallY = [data.height-5, data.height-5, data.height-30]
    data.wallWidth = [20, 20, 100]
    data.wallHeight = [20, 40, 10]
...
def movePlayer(dx, dy, data):
    ...
    if (wall != -1):
        data.playerX -= dx
        data.playerY -= dy
    if not isAboveGround(data):
        data.playerY = groundLevel(data)

# Note: this is more like interia now
def gravity(data):
    if isAboveGround(data):
        data.playerV -= data.gravityMag
    else:
        if data.playerU > 0:
            data.playerU -= 0.01
        else:
            data.playerU += 0.01
        data.playerV = 0
    movePlayer(data.playerU, data.playerV, data)

def groundLevel(data):
    return data.height - data.groundY + data.lineHeight

def isAboveGround(data):
    return (groundLevel(data) < data.playerY)
...
def keyPressed(event, data):
    ...
    if (event.keysym == "Left"):
        if data.playerU > data.playerUMin:
            data.playerU -= 2
    elif (event.keysym == "Right"):
        if data.playerU < data.playerUMax:
            data.playerU += 2
    elif (event.keysym == "space"): movePlayer(0, +30, data)
...
def redrawAll(canvas, data):
    ...
    canvas.create_rectangle(0, lineY, data.width, lineY+data.lineHeight,fill="black")
