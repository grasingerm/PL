...
template <typename T> void _pmergesort(T* to_sort, T* warr, const unsigned n) {
  if (n == 1) {
    ...
    const unsigned mid = n / 2;
    std::thread left  (_pmergesort<T>, to_sort, warr, mid);
    std::thread right (_pmergesort<T>, to_sort+mid, warr+mid, n-mid);
    left.join();
    right.join();
...
template <typename T> void pmergesort(T* to_sort, const unsigned n) {
  std::vector<T> temp_memory(n);
  _pmergesort<T>(to_sort, temp_memory.data(), n);
}
...
int main() {
  ...
  toc();
  print_array(svec1.data(), svec1.size());

  cout << "Mergesort with threading...\n";
  tic();
  pmergesort(svec2.data(), svec2.size());
  toc();
  print_array(svec2.data(), svec2.size());
