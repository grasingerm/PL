...
for h in hs
  ...
  for n in 1:K-1
    ...
    for l in 1:L
      u_temp[1, l] = asoln(0.0, ys[l], ts[n+1]);
      u_temp[M, l] = asoln(1.0, ys[l], ts[n+1]);
      u[1, l, n+1] = asoln(0.0, ys[l], ts[n+1]);
      u[M, l, n+1] = asoln(1.0, ys[l], ts[n+1]);
    end
    for m in 2:M-1
      u_temp[m, 1] = asoln(xs[m], 0.0, ts[n+1]);
      u_temp[m, L] = asoln(xs[m], 1.0, ts[n+1]);
...
println(@sprintf("%10.2lf %10.2lf %10s", hs[1], errs[1], "N/A"));
println(@sprintf("%10.2lf %10.2lf %10.lf", hs[2], errs[2], errs[1]/errs[2]));
println(@sprintf("%10.2lf %10.2lf %10.lf", hs[3], errs[3], errs[2]/errs[3]));
